...
async function masternodecapt_announce_post( masterNodeStatus ) {
  ...
  const baseUrl = 'https://greetr.herokuapp.com/mnode/announce'
  const options = {
    method: 'POST',
...
async function masternodecapt_announce( ) {
  ...
  for( let i = 0; i<masterNodes.length; i++ ) {
    ...
    const startedTime = procStatus ? procStatus.ExecMainStartTimestamp.toString() : null;
...
[Service]
ExecStart=/usr/local/bin/masternodecapt --service
Restart=always
User=nobody
