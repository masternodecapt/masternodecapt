...
async runCommand( cmd, verbose = false ) {
  const c = cmd.split(' ');
  const a = cmd.length > 1 ? cmd.slice( 1 ) : [];
  let errStr = '';
  let textStr = '';
  let err = false;
  var spawnPromise = spawn( c, a );
  if ( verbose ) {
...
function masternodecapt_statusTask() {
  ( async () => {
    const nodes = await masternodecapt_enumMasterNodes();
    for ( let i = 0; i < nodes.length; i++ ) {
      const status = await masternodecapt_getstatus( nodes[i] );
      console.log("Node ", nodes[i], " status ", status );
    }
  } )().then( () => {
    console.log("ok");
  } ).catch( err => {
...
async masternodecapt_getCliServiceStatus( cliName ) {
...
async masternodecapt_masterNodeProcessStatus( cliName ) {
  console.log("Starting service");
  cliName = cliName.toLowerCase();
  if ( !cliName ) {
    throw new Error('no cliName'); 
